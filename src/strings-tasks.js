...
function invertCase(str) {
  return str.split('').map(char => char.toUpperCase() === char ? char.toLowerCase() : char.toUpperCase()).join('');
}
...
function getCardId(value) {
  ...
  const suits = ['♣', '♦', '♥', '♠'];
  const card = value.split('')[0];
  const suit = value.split('')[1];
  return cards.indexOf(card) * 4 + suits.indexOf(suit);
}
